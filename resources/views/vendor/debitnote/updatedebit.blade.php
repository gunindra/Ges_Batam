...
<div class="container-fluid" id="container-wrapper" data-id="{{ $debitNote->id }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            let selectedinvoice = debitNote.invoice_id;
            var coas = @json($coas);
            ...
            $('#selectVendor').on('change', function () {
                var vendorId = $(this).val();  // Ambil vendor_id yang dipilih

                if (vendorId) {
                    $.ajax({
                        url: "{{ route('GetInvoiceUpdate') }}",
                        type: 'GET',
                        data: {
                            vendor_id: vendorId,  // Pastikan vendor_id dikirim dengan benar
                            selectedinvoice: selectedinvoice
                        },
                        success: function (response) {
                            $('#invoiceDebit').empty();  // Kosongkan dropdown invoice

                            if (response.success) {
                                // Tambahkan pilihan default
                                $('#invoiceDebit').append('<option value="" selected disabled>Pilih Invoice</option>');

                                // Tambahkan opsi invoice ke dropdown
                                $.each(response.SupInvoice, function (index, invoice) {
                                    var isSelected = (invoice.invoice_no === response.invoiceNo) ? 'selected' : '';
                                    $('#invoiceDebit').append('<option value="' + invoice.invoice_no + '" ' + isSelected + '>' + invoice.invoice_no + '</option>');
                                });

                                // Pilih invoice yang sudah ada (selected)
                                $('#invoiceDebit').val(response.invoiceNo).trigger('change');
                            } else {
            ...
            $('#updateDebit').click(function () {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function (response) {
                        $('#invoiceDebit').val(response.selectedInvoice).trigger('change');
